...
export function createOpenIdMongoAdapter(mongoConnection) {
  const collections = {};
  ...
  function getCollection(name) {
    ...
    collections[name] = mongoConnection.collection(`OPENID_${name}`);
    return collections[name].createIndexes([
      { key: { grantId: 1 } },
      { key: { expiresAt: 1 }, expireAfterSeconds: 0 },
    ]).then(() => collections[name]);
  }

  return class MongoAdapter {
    /**
     *
...
export default createOpenIdMongoAdapter;
